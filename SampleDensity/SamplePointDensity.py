...
class SamplePointDensity(VTKPythonAlgorithmBase):
    def __init__(self):
    ...
    def RequestInformation(self, request, inInfoVec, outInfoVec):
        ...
        for i in range(input0.GetPointData().GetNumberOfArrays()):
          self.vars.append(input0.GetPointData().GetArray(i).GetName())
        return 1
    ...
    def RequestData(self, request, inInfoVec, outInfoVec):
        ...
        if obj.GetClassName() == 'vtkImageData':
          ...
          ccode.SampleVTI(1, nSamples, dimensions, origin, spacing, pdf);
          ...
          pdf    = np.ascontiguousarray(vtu.PointData[pdf]).astype('f4')
          tets   = np.ascontiguousarray(vtu.Cells).astype('i4')
          ccode.SampleTetrahedra(1, nSamples, vtu.GetNumberOfPoints(), vtu.GetNumberOfCells(), points, pdf, tets)
          ...
          pdf    = dsa.numpy_support.vtk_to_numpy(vtr.GetPointData().GetArray(pdf)).astype('f4')
          dim = (len(x_array), len(y_array), len(z_array))
          ccode.SampleRectilinear(1, dim, x_array, y_array, z_array, pdf)

        else:
          print("SamplePointDensity requires vtkImageData or vtkUnstructuredGrid", obj.GetClassName())
          return 0
